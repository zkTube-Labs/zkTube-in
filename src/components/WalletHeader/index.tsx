...
const header = (
  <Link to="/wallet">
    <img
      ...
      style={{ padding: '0px', height: '34px', marginLeft: '20px' }}
    />
  </Link>
);
...
const WalletHeader = () => {
  ...
  return (
    <Nav
      ...
      header={header}
      footer={footer}
      defaultSelectedKeys={['My Wallet']}
      triggerType="hover"
    >
      <Item key="My Wallet" className={styles.size} style={{ marginLeft: '0px' }}>
        <Link to="/wallet/detail" style={{ marginTop: '-14px' }}>
          {' '}
          My Wallet
        </Link>
      </Item>
      <Item key="ZKTubeScan" className={styles.size} style={{ marginLeft: '5px' }}>
        <a href="https://rinkeby-browser.zktube.io/" target="_blank" style={{ marginTop: '-14px' }}>
          {' '}
          ZkTubeScan
        </a>
      </Item>
